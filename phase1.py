...
from ModelArchitecture.CharRNN import CharRNNClassic
import re
...
def classic_rnn_training(train_dataset, val_dataset, hidden_size, layer_amount, epochs, batch_size, learning_rate, vocab_size, embedding_size, window_size, loss_func, save_dir, char_to_idx, idx_to_char, cell_type, device, early_stopping_patience = 5, optimizer="adam"):
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    model_time = f"classic_{cell_type}_{timestamp}"
    ...
    run = wandb.init(
        ...
        name= model_time,
        config={
            "architecture": f"Char-RNN-Classic-{cell_type}",
            ...
            "cell_layer_amount": layer_amount,
        }
    ...
    net = CharRNNClassic(input_size = vocab_size, hidden_size = hidden_size ,output_size = vocab_size, embedding_size= embedding_size, model=cell_type, n_layers = layer_amount)
    net = net.to(device)
    ...
    for epoch in range(epochs):
        ...
        if(avg_val_loss < best_val_loss):
            ...
            file_name = f"classic_rnn_{cell_type}_epoch_{epoch+1}_{timestamp}.pt"
...
def main():
    ...
    #GRU
    best_gru_epoch, best_gru_val_loss = classic_rnn_training(train_dataset=train_dataset, val_dataset=val_dataset, hidden_size=hidden_size, layer_amount=layer_amount, epochs=epochs, batch_size=batch_size, learning_rate=learning_rate, vocab_size=vocab_size, embedding_size=embedding_size, window_size=window_size, loss_func=loss_func, save_dir=save_dir, char_to_idx=char_to_idx, idx_to_char=idx_to_char, cell_type="gru", device=device)

    #LSTM
    best_lstm_epoch, best_lstm_val_loss = classic_rnn_training(train_dataset=train_dataset, val_dataset=val_dataset, hidden_size=hidden_size, layer_amount=layer_amount, epochs=epochs, batch_size=batch_size, learning_rate=learning_rate, vocab_size=vocab_size, embedding_size=embedding_size, window_size=window_size, loss_func=loss_func, save_dir=save_dir, char_to_idx=char_to_idx, idx_to_char=idx_to_char, cell_type="lstm", device=device)

    print(f"BEST GRU LOSS AT EPOCH {best_gru_epoch}: {best_gru_val_loss} ")
    print(f"BEST LSTM LOSS AT EPOCH {best_lstm_epoch}: {best_lstm_val_loss} ")
